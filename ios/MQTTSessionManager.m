...
- (void)setSubscription:(NSString *)topic
                    qos:(NSNumber *)qos
    ...
    if (self.state == MQTTSessionManagerStateConnected){
        ...
        MQTTQosLevel qosNumber = qos.unsignedIntValue;
        __weak MQTTSessionManager *weakSelf = self;
    ...
    self.internalSubscriptions = [NSDictionary dictionaryWithObject:qos forKey:topic];
}
...
- (UInt16)sendData:(NSData *)data
    ...
    publishHandler:(MQTTPublishHandler)handler{
    if (self.state != MQTTSessionManagerStateConnected) {
        [self connectToLast:nil];
    }
    UInt16 msgId = [self.session publishData:data
...
- (void)unsetSubscriptions:(NSArray<NSString *> *)topics
    ...
    if (self.state == MQTTSessionStatusConnected){
        __weak MQTTSessionManager *weakSelf = self;
        [self.session unsubscribeTopics:topics unsubscribeHandler:^(NSError *error) {
            MQTTSessionManager *strongSelf = weakSelf;
            if (!error) {
